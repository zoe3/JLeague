...
```{r outputfilename}
outputfilename <- "submit_20171210_1_lm.csv"
```
...
param <- c("capa", "pre_rank.home", "pre_rank.away", "media", "section", "home_team", "away_team","week","rain", "pre_win_rate.home", "pre_win_rate.away", "heat")
explain <- c("スタジアムの収容人数", "前節のランキング(home)","前節のランキング(away)", "tv局数", "節", "ホームチーム", "アウェイチーム", "1(土日祝)、0(平日)", "1(雨)、0(雨以外)","ピタゴラス勝率(home)","ピタゴラス勝率(away)","後半の下位争い")
...
###使用変数の作成
lm_train<-dplyr::select(train_train, id, y, stage, param)
lm_test<-dplyr::select(train_test, id, y, stage, param)
...
### testに当てはめ
by_stage_test <- by_stage_test %>%
    dplyr::inner_join(by_stage_train %>% select(stage, model),by="stage")
...
## RMSEの計算
test.pred <- unnest(by_stage_test, pred, resids)
sqrt(mean((test.pred$y - test.pred$pred)^2))

## broom
glance <- by_stage_test %>%
...
sample_submit <- read_csv("../motodata/sample_submit.csv", col_names=c("id","pred"))
nrow(sample_submit)

nrow(test)
test <- test %>%
    dplyr::inner_join(sample_submit%>% select(id), by="id")
nrow(test)

###使用変数の作成
lm_train<-dplyr::select(train, id, y, stage, param)
lm_test<-dplyr::select(test, id, stage, param)
...
###submit形式に整える
test.pred <- unnest(by_stage_test, pred)
submit <- test.pred %>%
